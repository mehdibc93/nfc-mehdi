...
export function StatsPage() {
  ...
  const stats = useMemo(() => {
    ...
    filteredEvents.forEach((event) => {
      ...
      if (!stat) {
        stat = { key, name: `${event.dish_name} (supprimé)`, views: 0, addToCart: 0, removedFromCart: 0, purchases: 0 };
        map.set(key, stat);
      }
  ...
  if (!restaurant) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <p className="text-stone-500">Créez votre restaurant avant de consulter les statistiques.</p>
        <Link
          to="/dashboard"
    ...
    <div className="min-h-screen pb-20">
      <header className="sticky top-0 z-40 border-b border-stone-900/5 bg-[#f6f8fb]/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-5xl flex-col gap-1 px-4 py-5 sm:px-8">
          ...
          <p className="font-display text-lg font-semibold text-stone-900">Statistiques — {restaurant.name}</p>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-3xl border border-stone-200/70 bg-white p-6 shadow-soft">
            <p className="text-xs uppercase tracking-[0.28em] text-stone-400">Vues de plats</p>
            <p className="mt-2 text-3xl font-bold text-stone-900">{totals.views}</p>
          </div>
          <div className="rounded-3xl border border-stone-200/70 bg-white p-6 shadow-soft">
            <p className="text-xs uppercase tracking-[0.28em] text-stone-400">Ajouts au panier</p>
          ...
          <div className="rounded-3xl border border-navy-300/25 bg-navy-300/8 p-6">
            <p className="text-xs uppercase tracking-[0.28em] text-navy-700">Commandes payées</p>
            ...
            <table className="w-full min-w-[680px] text-left text-sm">
              <thead>
                <tr className="border-b border-stone-200 text-xs uppercase tracking-[0.2em] text-stone-400">
                  ...
                  <th className="px-5 py-4">Vues</th>
                  <th className="px-5 py-4">Ajouts panier</th>
                  <th className="px-5 py-4">Retirés du panier</th>
                  <th className="px-5 py-4">Achetés</th>
                  <th className="px-5 py-4">Abandonnés</th>
        ...
        <p className="text-xs text-stone-400">
          « Abandonnés » = ajouts au panier qui n'ont pas abouti à un paiement (peut inclure des commandes encore en
          cours). Basé sur les 5000 derniers événements enregistrés.
        </p>
      </main>
    </div>
    </PinSectionGate>
